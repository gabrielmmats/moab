...
void Range::get_entity_vector(EntityHandle *entity_vector)
{
  int i_vector = 0;
  //if this is empty
  ...
  for(current_node=mHead.mNext; current_node != &mHead; current_node=current_node->mNext)
  {
    if (current_node->first == current_node->second)
    {
      entity_vector[i_vector]=current_node->first;
      i_vector++;
    }
    else
    {
      for(EntityHandle i = current_node->first; i <= current_node->second; i++)
      {
        entity_vector[i_vector] = i;
        i_vector++;
      }
    }
  }
...
void Range::get_entity_vector_key(EntityHandle *entity_vector, const int *key_vector, const int key_size)
{
  int i_vector=0;
  int first_position=0; //position of node->first if the entire range was an array
  int current_key = key_vector[i_vector];
  PairNode* current_node=mHead.mNext;
  int node_size = current_node->second - current_node->first + 1;
  if(&mHead == mHead.mNext)
  {
    return;
  }
  for (i_vector=0; i_vector<key_size;)
  {
    /*
    if the searched key is in this node range
    */
    if(current_key >= first_position && current_key <  first_position + node_size){
      entity_vector[i_vector] = current_node->first + current_key - first_position;
      i_vector++;
      current_key = key_vector[i_vector];
    }
    /*
    if the searched key is after this node range
    */
    else if(current_key >= first_position + node_size)
    {
      current_node = current_node->mNext;
      first_position = first_position + node_size;
      node_size = current_node->second - current_node->first + 1;
    }
    /*
    the searched key is before this node range
    */
    else
    {
      current_node = current_node->mPrev;
      node_size = current_node->second - current_node->first + 1;
      first_position = first_position - node_size;
    }
  }
...
void Range::insert_entity_vector(EntityHandle *entity_vector, const int vector_size)
{
  int i;
  Range::iterator hint;
  hint = iterator(mHead.mNext, mHead.mNext->first);
  for(i=0; i<vector_size; i++)
  {
    hint = insert(hint, entity_vector[i]);
  }
}
